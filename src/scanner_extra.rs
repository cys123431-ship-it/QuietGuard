...
fn scan_service_dlls(out: &mut Vec<String>, rules: &Rules) {
    let text = command_output("reg", &["query", "HKLM\\SYSTEM\\CurrentControlSet\\Services", "/s", "/v", "ServiceDll"]);
    if text.is_empty() { out.push("[정보] ServiceDll: 조회 결과 없음".into()); return; }
...
fn scan_ifeo_debuggers(out: &mut Vec<String>, rules: &Rules) {
    let text = command_output("reg", &["query", "HKLM\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options", "/s", "/v", "Debugger"]);
    if text.is_empty() { out.push("[정상] IFEO Debugger: 등록 항목 없음".into()); return; }
    let mut total = 0usize;
    let mut suspicious = 0usize;
    for line in text.lines().map(str::trim) {
        let lower = line.to_ascii_lowercase();
        if !lower.contains("debugger") || !(line.contains("REG_SZ") || line.contains("REG_EXPAND_SZ")) { continue; }
        total += 1;
...
fn scan_bits_jobs(out: &mut Vec<String>, rules: &Rules) {
    ...
    if text.is_empty() { out.push("[정보] BITS: 조회 불가 또는 작업 없음".into()); return; }
    let mut suspicious = 0usize;
    for line in text.lines().map(str::trim) {
        if rules.autorun_is_suspicious(line) {
            suspicious += 1;
            if suspicious <= 5 { out.push(format!("[주의] BITS 관련 경로/명령: {}", compact(line, 120))); }
        }
    }
    if suspicious == 0 { out.push("[정보] BITS: 의심 경로/명령 패턴 없음".into()); }
    else { out.push(format!("[정보] BITS: 의심 패턴 {}건", suspicious)); }
}

fn scan_winsock(out: &mut Vec<String>, rules: &Rules) {
    let text = command_output("netsh", &["winsock", "show", "catalog"]);
    if text.is_empty() { out.push("[정보] Winsock 카탈로그를 읽지 못함".into()); return; }
    let mut suspicious = 0usize;
    for line in text.lines().map(str::trim) {
        if rules.path_is_suspicious(line) {
            suspicious += 1;
            if suspicious <= 5 { out.push(format!("[주의] Winsock 공급자 경로: {}", compact(line, 120))); }
        }
    }
    if suspicious == 0 { out.push("[정보] Winsock: 사용자 쓰기 경로 기반 공급자 흔적 없음".into()); }
    else { out.push(format!("[정보] Winsock: 주의 패턴 {}건", suspicious)); }
}
...
fn scan_wmi_persistence(out: &mut Vec<String>, rules: &Rules) {
    ...
    let entries: Vec<&str> = text.lines().map(str::trim)
        .filter(|l| l.starts_with("CMD|") || l.starts_with("SCRIPT|")).collect();
    if entries.is_empty() { out.push("[정보] WMI 영구 소비자: 발견 없음 또는 조회 불가".into()); return; }
    out.push(format!("[확인] WMI 영구 이벤트 소비자 {}개", entries.len()));
    for entry in entries.iter().take(5) {
        if rules.autorun_is_suspicious(entry) { out.push(format!("[주의] WMI: {}", compact(entry, 120))); }
        else { out.push(format!("  - {}", compact(entry, 120))); }
    }
}

fn scan_browser_policies(out: &mut Vec<String>) {
    const TARGETS: &[(&str, &str, &[&str])] = &[
        ("Chrome 정책", "HKCU\\Software\\Policies\\Google\\Chrome", &["HomepageLocation", "DefaultSearchProviderSearchURL", "RestoreOnStartupURLs"]),
        ("Edge 정책", "HKCU\\Software\\Policies\\Microsoft\\Edge", &["HomepageLocation", "DefaultSearchProviderSearchURL", "RestoreOnStartupURLs"]),
        ("Chrome 시스템 정책", "HKLM\\Software\\Policies\\Google\\Chrome", &["HomepageLocation", "DefaultSearchProviderSearchURL", "RestoreOnStartupURLs"]),
        ("Edge 시스템 정책", "HKLM\\Software\\Policies\\Microsoft\\Edge", &["HomepageLocation", "DefaultSearchProviderSearchURL", "RestoreOnStartupURLs"]),
    ];
    ...
    let ie = command_output("reg", &["query", "HKCU\\Software\\Microsoft\\Internet Explorer\\Main"]);
    ...
    if found == 0 { out.push("[정보] 브라우저 시작/검색 강제 정책 흔적 없음".into()); }
}
