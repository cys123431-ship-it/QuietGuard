...
pub fn run_watcher() {
    unsafe {
        ...
        // WaitForMultipleObjects supports up to 64 handles; this stays far below that
        // while avoiding a worker thread per watched key.
        ...
        add_reg_watch(&mut watches, "시스템 App Paths", HKEY_LOCAL_MACHINE,
            "Software\\Microsoft\\Windows\\CurrentVersion\\App Paths", true);
        add_reg_watch(&mut watches, "사용자 Explorer 실행 제한 정책", HKEY_CURRENT_USER,
            "Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\Explorer", true);
        add_reg_watch(&mut watches, "시스템 Explorer 실행 제한 정책", HKEY_LOCAL_MACHINE,
            "Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\Explorer", true);
