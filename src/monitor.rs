...
extern "system" {
    ...
    fn RegNotifyChangeKeyValue(hKey: HKEY, bWatchSubtree: i32, dwNotifyFilter: DWORD, hEvent: HANDLE, fAsynchronous: i32) -> i32;
    fn RegCloseKey(hKey: HKEY) -> i32;
}

struct RegWatch { label: &'static str, key: HKEY, event: HANDLE, subtree: bool }
struct FileWatch { label: &'static str, path: PathBuf, stamp: Option<u128> }

pub fn start_background() -> String {
    if is_running() { return "[정보] 실시간 감시는 이미 실행 중입니다.".into(); }
    let exe = match env::current_exe() { Ok(v) => v, Err(e) => return format!("[오류] 실행 파일 경로 확인 실패: {}", e) };
    let mut cmd = Command::new(exe);
    cmd.arg("--watch");
    cmd.creation_flags(CREATE_NO_WINDOW);
    match cmd.spawn() { Ok(_) => "[완료] 저메모리 실시간 감시를 시작했습니다.".into(), Err(e) => format!("[오류] 실시간 감시 시작 실패: {}", e) }
}
...
pub fn request_stop() -> String {
    unsafe {
        ...
        if event.is_null() { return "[정보] 실행 중인 실시간 감시가 없습니다.".into(); }
        let ok = SetEvent(event);
        CloseHandle(event);
        if ok != 0 { "[완료] 실시간 감시 중지를 요청했습니다.".into() } else { "[오류] 실시간 감시 중지 신호를 보내지 못했습니다.".into() }
    }
}
...
pub fn run_watcher() {
    unsafe {
        ...
        for watch in watches { RegCloseKey(watch.key); CloseHandle(watch.event); }
        CloseHandle(stop_event); CloseHandle(mutex);
...
unsafe fn add_reg_watch(watches: &mut Vec<RegWatch>, label: &'static str, root: HKEY, subkey: &str, subtree: bool) {
    ...
    if RegOpenKeyExW(root, sub.as_ptr(), 0, KEY_NOTIFY, &mut key) != 0 || key.is_null() { return; }
    let event = CreateEventW(null_mut(), 0, 0, null());
    if event.is_null() { RegCloseKey(key); return; }
    let watch = RegWatch { label, key, event, subtree };
    if !arm_watch(&watch) { RegCloseKey(key); CloseHandle(event); return; }
    watches.push(watch);
}

unsafe fn arm_watch(watch: &RegWatch) -> bool {
    RegNotifyChangeKeyValue(watch.key, if watch.subtree { 1 } else { 0 }, REG_NOTIFY_CHANGE_NAME | REG_NOTIFY_CHANGE_LAST_SET, watch.event, 1) == 0
}
...
fn build_file_watches() -> Vec<FileWatch> {
    ...
    if let Ok(appdata) = env::var("APPDATA") { add_file_watch(&mut watches, "사용자 Startup", PathBuf::from(appdata).join("Microsoft\\Windows\\Start Menu\\Programs\\Startup")); }
    if let Ok(programdata) = env::var("ProgramData") { add_file_watch(&mut watches, "전체 사용자 Startup", PathBuf::from(programdata).join("Microsoft\\Windows\\Start Menu\\Programs\\StartUp")); }
    watches
}
