...
pub fn run_extra_scan5() -> Vec<String> {
    ...
    out.push("--- 추가 점검: 네트워크/정책/프로그램 연결 ---".into());
...
fn scan_winhttp_proxy(out: &mut Vec<String>) {
    ...
    if lower.contains("direct access") || text.contains("직접 액세스") || text.contains("직접 액세스(프록시 서버 없음)") {
...
fn scan_app_paths(out: &mut Vec<String>, rules: &Rules) {
    ...
    for key in KEYS {
        ...
        for line in text.lines().map(str::trim) {
            ...
            if rules.autorun_is_suspicious(line) {
                suspicious += 1;
                out.push(format!("[주의] App Paths 의심 경로: {}", compact(line, 160)));
            }
        }
    }
    out.push(format!("[정보] App Paths 값 {}개 확인, 의심 경로 {}개", total, suspicious));
}
...
fn scan_ipsec_policy(out: &mut Vec<String>, rules: &Rules) {
    ...
    for key in KEYS {
        ...
        if !suspicious.is_empty() {
            out.push(format!("[주의] 정책 영역 의심 경로/명령 {}개: {}", suspicious.len(), key));
...
fn command_output(program: &str, args: &[&str]) -> String {
    let output = hidden_output(program, args);
    match output {
