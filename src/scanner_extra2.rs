...
fn scan_environment_and_logon(out: &mut Vec<String>, rules: &Rules) {
    ...
    let machine_env = command_output("reg", &[
    ...
    for (scope, text) in [("사용자", user_env.as_str()), ("시스템", machine_env.as_str())] {
        for line in text.lines().map(str::trim) {
            ...
            if rules.path_is_suspicious(line) || rules.command_is_suspicious(line) {
                suspicious += 1;
                if suspicious <= 8 {
                    out.push(format!("[주의] {} 환경변수: {}", scope, compact(line, 120)));
                }
            }
        }
    }

    let logon = command_output("reg", &[
        "query", "HKCU\\Environment", "/v", "UserInitMprLogonScript"
    ]);
    if let Some(line) = logon.lines().map(str::trim)
        .find(|l| l.to_ascii_lowercase().contains("userinitmprlogonscript")) {
        if line.contains("REG_SZ") || line.contains("REG_EXPAND_SZ") {
            if rules.autorun_is_suspicious(line) {
                out.push(format!("[주의] UserInitMprLogonScript: {}", compact(line, 120)));
            } else {
                out.push(format!("[확인] UserInitMprLogonScript 등록: {}", compact(line, 120)));
            }
        }
    }

    let startup = command_output("reg", &[
        "query",
        "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\User Shell Folders",
        "/v", "Startup"
    ]);
    if let Some(line) = startup.lines().map(str::trim)
        .find(|l| l.to_ascii_lowercase().contains("startup")) {
        if rules.path_is_suspicious(line) {
            out.push(format!("[주의] Startup 폴더 경로 변경 의심: {}", compact(line, 120)));
        }
    }
...
fn scan_file_associations(out: &mut Vec<String>, rules: &Rules) {
    const CLASSES: &[&str] = &[
        "exefile", "comfile", "batfile", "cmdfile", "mscfile", "htmlfile",
        "http", "https"
    ];
...
fn scan_uninstall_entries(out: &mut Vec<String>, rules: &Rules) {
    ...
    for &root in ROOTS {
        ...
        for line in text.lines().map(str::trim) {
            ...
            if !(lower.contains("uninstallstring") || lower.contains("quietuninstallstring")) {
                continue;
            }
            if !(line.contains("REG_SZ") || line.contains("REG_EXPAND_SZ")) { continue; }
            strings += 1;
            if rules.autorun_is_suspicious(line) {
                suspicious += 1;
                if suspicious <= 8 {
                    out.push(format!("[주의] 제거 명령 경로: {}", compact(line, 120)));
                }
            }
        }
    }
...
fn scan_browser_shortcuts(out: &mut Vec<String>, rules: &Rules) {
    ...
    for line in text.lines().map(str::trim).filter(|l| l.starts_with("LNK|")) {
        total += 1;
        let lower = line.to_ascii_lowercase();
        let has_url_arg = lower.contains("|http://") || lower.contains("|https://")
            || lower.contains(" http://") || lower.contains(" https://");
        if rules.autorun_is_suspicious(line) {
            suspicious += 1;
            if suspicious <= 8 {
                out.push(format!("[주의] 브라우저 바로가기: {}", compact(line, 120)));
            }
        } else if has_url_arg {
    ...
    for &root in ROOTS {
        ...
        for line in text.lines().map(str::trim) {
            ...
            if rules.path_is_suspicious(line) || rules.command_is_suspicious(line) {
                suspicious += 1;
                if suspicious <= 6 {
                    out.push(format!("[주의] IE ElevationPolicy: {}", compact(line, 120)));
                }
            }
        }
    }
